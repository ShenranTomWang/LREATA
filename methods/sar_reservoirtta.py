...
class SAR_ReservoirTTA(TTAMethod):
    ...
    def __init__(self, cfg, model, num_classes):
        ...
        self.ema = [None] * cfg.RESERVOIRTTA.MAX_NUM_MODELS
        # setup loss function
    ...
    def forward_and_adapt(self, x):
        ...
        if self.ema[self.reservoir.model_idx] is not None:
            if self.ema[self.reservoir.model_idx] < self.reset_constant_em:
                logger.info(f"soup-id: {self.reservoir.model_idx}, ema {self.ema[self.reservoir.model_idx]:.3f} < threshold {self.reset_constant_em:.3f}, now reset the model")

                ############ Reset kth Soup Model and its optimizer #########
                self.reservoir.reset_kth_model(self.optimizer_state, model_idx=self.reservoir.model_idx)
                self.ema[self.reservoir.model_idx] = None
                ############ Reset kth Soup Model #########
    ...
    def setup_optimizer(self):
        ...
        if "vit" in architecture_name or "swin_" in architecture_name:
